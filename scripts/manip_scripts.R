...
slice(df.1, 10)
slice(df.1, 1:6)   ## slice peut aussi afficher le même résultat que head(df.1)
head(df.1)
...
unique(df.1$month)
filter(df.1, month==1)
filter(df.1, dep_time>500| dep_time<1000)
filter(df.1, dep_time>500 & dep_time<1000)%>%
  #select(df.1, det_time)
    #summary(df.1$dep_time)

filter(df.1, distance==max(distance))
...
select(df.1, starts_with("dep_"))
select(df.1, ends_with("e"))
select(df.1, contains("ou"))
...
select(df.1, all_of(c("year", "month", "day","beni")))
select(df.1, any_of(c("year", "month", "day","beni")))
...
str(df.1)
select(df.1, where(is.character))
select(df.1, where(is.numeric))
...
select(df.2, name, lat, lon,everything())
...
dplyr::arrange(df.1, month)
dplyr::arrange(df.1, dep_delay)
...
select(df.1, distance, distance_km, vitesse)
...
df.1$month_name <- recode_factor(flights$month,
                                    "1" = "Jan",
                                    "2" = "Feb",
                                    "3" = "Mar",
                                    "4" = "Apr",
